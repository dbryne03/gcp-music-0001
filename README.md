...
Ingests chart, artist, and track data from three external sources on the first of each month, unifies them in BigQuery via dbt, and surfaces insights through a Data Studio dashboard. Infrastructure runs entirely on GCP, provisioned via idempotent gcloud CLI scripts managed by GitHub Actions.
